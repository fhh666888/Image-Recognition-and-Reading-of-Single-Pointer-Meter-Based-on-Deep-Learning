...
class MeterReader(object):
    ...
    def __call__(self, image,point_mask,dail_mask,word_mask,number,std_point,circle_center):

        img_result = image.copy()
        value=self.find_lines(img_result,point_mask,dail_mask,number,std_point,circle_center)
    ...
    def find_lines(self,ori_img,pointer_mask,dail_mask,number,std_point,circle_center):
        save_path = 'yolov5-5.0/runs/cut2/'
        # 实施骨架算法
        ...
        if str(pointer_lines) != 'None':
            ...
            if number!=None and number[0]!="":
                ...
                # if two_value > 40.0 and two_value < 100:  ############
                #     two_value = two_value/10
                # elif two_value > 100:
                #     two_value = two_value/100
                # else:
                #     pass
            else:
            ...
            filename_last = str(self.n) + ".jpg"
            self.n=self.n+1
            #cv2.imwrite(os.path.join(save_path, filename_last), ori_img)
            cv2.imshow("result",ori_img)
            cv2.waitKey(0)
        else:
            font = cv2.FONT_HERSHEY_SIMPLEX
            ori_img = cv2.putText(ori_img, 'None', (30, 30), font, 1.2, (255, 0, 255), 2)
...
if __name__ == '__main__':
    tester = MeterReader()
    root = 'D:/detection/yolov5-5.0/runs/cut1/'
